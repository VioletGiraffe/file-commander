...
{
	if (_controller)
		_controller->iconProvider().removeIconsChangedListener(this);
...
void CPanelWidget::init(CController* controller, CShellOperationRunner& shellOperations)
{
	...
	_controller->iconProvider().addIconsChangedListener(this);
}
...
void CPanelWidget::volumesChanged(const std::vector<VolumeInfo>& drives, Panel p, bool drivesListOrReadinessChanged) noexcept
{
	...
	if (drivesListOrReadinessChanged)
	{
		...
		for (const VolumeInfo& volume: drives)
		{
...
#ifdef _WIN32
			const QString name = QString{ volume.rootObjectInfo.fullAbsolutePath() }.remove(QL1(":/"));
#else
			...
			diskButton->setIcon(_controller->iconProvider().preciseIconBlocking(volume.rootObjectInfo.fullAbsolutePath(), volume.rootObjectInfo.modificationTime()));
...
// Only the tab on screen can repaint; switching to another re-attaches the shared view, which refetches every row.
...
void CPanelWidget::onAllIconsInvalidated()
{
	if (_model)
		_model->onAllIconsInvalidated();
}
