...
the newest MSVC installed: qmake gates the compiler flags and the `PlatformToolset` it writes into the projects on
the `cl` it probes, and the build takes the toolset from there. Pinning an older toolset therefore also means
putting a matching `cl` in front of the probe.
